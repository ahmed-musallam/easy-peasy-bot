...
var REGEX = /@([^ ]+)[ ]*\+\+/g
controller.hears(REGEX.source, 'direct_message', function (bot, message) {
  console.log(JSON.stringify(message))
  controller.storage.users.get(message.user, function (err, data) {
    if (err) console.log(`could not find user ${message.user}`)
    data = Object.assign({ score: 0 }, data)
    const userName = message.match[1]
    const newScore = data.score + 1
    controller.storage.users.save({ id: userName, score: newScore }, function (err) {
      if (err) console.log(err)
      bot.reply(message, `[@${userName}] now at ${newScore} points`)
    })
  })
})
